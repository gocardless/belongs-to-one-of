...
RSpec.shared_examples "belongs to at most one organisation" do
  ...
  context "when organisation is specified correctly" do
    it "does not throw error" do
      expect(competitor.errors[:base]).to_not include("model must belong to at most one organisation")
    end
  end
  ...
  context "when multiple resources are specified" do
    ...
    it "throws error" do
      expect(competitor.errors[:base]).to include("model must belong to at most one organisation")
    end
  end
end
